...
{return;}});}});})(jQuery,this);(function($,window,undefined){if(EA.mobile&&EA.mobile.agents){for(i in EA.mobile.agents){var regex=new RegExp(EA.mobile.agents[i],"i");if(navigator.userAgent.match(regex)){$('#mod-mobile-redirect').show();break;}}}})(jQuery,this);;(function($,window,undefined){$('#bd').append('<span class="hk-bg"></span>');if(!$.browser.msie)
{EA.flashLoaded=function(swfId)
{switch(swfId)
{case('background'):{$('#bd > .hk-bg').css('background-image','none');break;}}};EA.flashEvent=function(swfId,action)
{switch(swfId)
{case('menu'):{switch(action)
{case('loaded'):{$('#bd > .hk-anim').css('zIndex','998');$.flashProxy.getSWF('anim-flash').play();break;}
case('animationComplete'):{$('#bd > .hk-anim').remove();break;}}
break;}}};var path=window.location.pathname;if(path.charAt(path.length-1)==='/')
{path=path.substr(0,path.length-1);}
var pathParts=path.split("/");if(pathParts[pathParts.length-1]!=='intro')
{var swfPath=EA.skinPath+'swf/background/';var ver=10;var expressPath=null;var flashvars=null;var params={allowScriptAccess:'always',bgcolor:'#000000',menu:'false',scale:'noborder',wmode:'transparent'};var attributes={id:'bg-flash'};var callbacks=null;var flashEvents=null;$('#bd > .hk-bg').flash(swfPath+'background.swf','100%','100%',ver,expressPath,flashvars,params,attributes,callbacks,flashEvents);swfPath=EA.skinPath+'swf/loadanim/';attributes.id='anim-flash';$('<span class="hk-anim"></span>').appendTo('#bd').flash(swfPath+'rollovers.swf','100%','100%',ver,expressPath,flashvars,params,attributes,callbacks,flashEvents);}}})(jQuery,this);;(function($,window,undefined){$('#mod-editorial-pod').append('<span class="hk-1"></span><span class="hk-2"></span><span class="hk-3"></span>');})(jQuery,this);;(function($,window,undefined){$('#mod-preorder').append('<span class="hk-1"></span><span class="hk-2"></span><span class="hk-3"></span>');$('#mod-preorder').find('.mod-content li a').each(function(idx)
{var $this=$(this);var content=$this.html().toLowerCase();$this.attr('href',(content.indexOf('xbox')>=0)?'http://marketplace.xbox.com/en-US/Product/MicroBot/66acd000-77fe-1000-9115-d80258410a9c/':(content.indexOf('psn')>=0)?'http://us.playstation.com/games-and-media/games/microbot-ps3.html':'#');});})(jQuery,this);;(function($,window,undefined){$(document).ready(function()
{var $ticker=$('#carousel-review-ticker');$ticker.find('li blockquote').css('width',$ticker.width());$ticker.carousel({step:1,visible:1,loop:true,effect:'fade',control:false,autoSlide:true,autoSlideInterval:8000});});})(jQuery,this);;(function($,window,undefined){$('#tabs-media-listing').tabs({cache:true,spinner:''});$('#mod-game-media-listing').append('<span class="hk-1"></span><span class="hk-2"></span><span class="hk-3"></span>');})(jQuery,this);;(function($,window,undefined){$('#mod-newsletter').append('<span class="hk-1"></span><span class="hk-2"></span><span class="hk-3"></span>');$(window).load(function()
...
{$('#mod-newsletter .mod-header .message .error-message').html(message);$elms.each(function(idx)
...
{$elm.removeClass('disabled');}}});})(jQuery,this);;(function($,window,undefined){$('#mod-share-links').append('<span class="hk-1"></span><span class="hk-2"></span><span class="hk-3"></span>');})(jQuery,this);function populate_latest_news(data){var items=[];$.each(data.responseData.feed.entries,function(key,val){items.push('<li><div class="news-body"><a href="'+val.link+'">'+val.contentSnippet.replace(/(\\u0026nbsp;|\\n)*/g,'').replace(/(&nbsp;)+/g,'')+'</a></div></li>');});$('#mod-latest-news .mod-content').html('<ul>'+items.join('')+'</ul>');$('#mod-latest-news .mod-content ul').resetListClasses();}
(function($,window,undefined){$(document).ready(function(){if(EA.externalBlog&&EA.externalBlog.src)
$(document.body).append('<script type="text/javascript" src="https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&q='+EA.externalBlog.src+'&callback=populate_latest_news" />');});})(jQuery,this);;(function($,window,undefined){$('#mod-comments').append('<span class="hk-1"></span><span class="hk-2"></span><span class="hk-3"></span>');})(jQuery,this);;(function($,window,undefined){$('#mod-facebook-fans').append('<span class="hk-1"></span><span class="hk-2"></span><span class="hk-3"></span>');})(jQuery,this);