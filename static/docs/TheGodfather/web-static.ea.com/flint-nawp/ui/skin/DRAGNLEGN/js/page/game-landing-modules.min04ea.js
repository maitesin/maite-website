...
{return;}});}});})(jQuery,this);(function($,window,undefined){if(EA.mobile&&EA.mobile.agents){for(i in EA.mobile.agents){var regex=new RegExp(EA.mobile.agents[i],"i");if(navigator.userAgent.match(regex)){$('#mod-mobile-redirect').show();break;}}}})(jQuery,this);;(function($,window,undefined){if($("#mod-blog-archives .mod-content > div").size()>2){$("#mod-blog-archives .mod-content > div:eq(1)").addClass("last");$("#mod-blog-archives .mod-content > div:eq(2)").removeClass("last").addClass("first");$("#mod-blog-archives").after($("#mod-blog-archives").clone().addClass("mod-blog-archives-clone"));$("#mod-blog-archives:eq(0) .mod-content > div:gt(1)").empty();$(".mod-blog-archives-clone .mod-content > div:lt(2)").empty();}})(jQuery,this);;(function($,window,undefined){if($.browser.msie)
{var $gusSubnav=$('#mod-gus .gus-items li .sub-nav');$gusSubnav.each(function(i)
{$(this).append('<div class="hk1" style="width:'+$(this).outerWidth()+'px;"><span></span></div>');});}})(jQuery,this);(function($)
{$('#mod-countdown').bind('countdownTick',function(evt,data)
{var days=data.days.toString();while(days.length<3){days='0'+days;}
days=days.split('');var hours=data.hours.toString();while(hours.length<2){hours='0'+hours;}
hours=hours.split('');var minutes=data.minutes.toString();while(minutes.length<2){minutes='0'+minutes;}
minutes=minutes.split('');$(this).find('.days > ul > li').each(function(idx)
...
{var hour=hours[idx];var $hour=$(this);$hour.attr('title',hour).find('span').text(hour);$hour[0].className=$hour[0].className.replace(/\s?n\-\n{1}/g,'');$hour.addClass('n-'+hour);});$(this).find('.minutes > ul > li').each(function(idx)
{var minute=minutes[idx];var $minute=$(this);$minute.attr('title',minute).find('span').text(minute);$minute[0].className=$minute[0].className.replace(/\s?n\-\n{1}/g,'');$minute.addClass('n-'+minute);});}).bind('countdownComplete',function(evt)
{$('#mod-countdown > .mod-header > h3,#mod-countdown > .mod-content').hide();$('#mod-countdown .complete').show();});})(jQuery);;(function($,window,undefined){$(document).ready(function()
...
{var $carousel=$('#carousel-preorder');if($carousel.find('> ul > li').length>1)
{$carousel.carousel({step:1,visible:3});}});})(jQuery,this);;(function($,window,undefined){$(document).ready(function()
...
$(document.body).append('<script type="text/javascript" src="https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&q='+EA.externalBlog.src+'&callback=populate_latest_news" />');});})(jQuery,this);;(function($,window){if(EA){EA.baseDropdown=true;}})(jQuery,this);;(function($,window,undefined){var $likebox=$('#mod-facebook-fans .mod-content iframe');if($likebox.length>0){var width=$likebox.parent().width();if($likebox.width()!=width){$likebox[0].src=$likebox[0].src.replace(/width=[0-9]+/,'width='+width);$likebox.css('width',width);}}})(jQuery,this);(function($,window,undefined){$('#mod-twitter-feed').append('<span class="hk-bg"></span>');})(jQuery,this);